...
# Let's go for 2 hrs, 1hr would be fine too.
...
Let's simulate the 3-state HMMs, and then fit it to be able to identify the behavioural states.

<<simData>>=
# Simulate the model
sealSim <- simData(model=sealHMM3s, nbAnimals =1)
...
Looks similar, except maybe that the real seal goes back and forth to some of the same locations, something not captured in the simulation.
...
Covariates can affect the state-dependent movement, by which I mean the characteristics of the movement in a given state. So for example, the animal might make all the behaviours at night and during the day, but at night it might do them with smaller steps. Here, we are going to look at whether the seal is more or less directed at night. To do this, we are going to change the argument \texttt{DM}. 
...
par0diurnDM <- getPar0(model=sealHMM3s, formula=~1, DM=diurnDM)
...
The function \texttt{cosinor} internally creates the covariates $\cos(2 \pi \; \text{hour}/\text{period})$ and $\sin(2 \pi \; \text{hour}/\text{period})$.
...
points(sealSimB[,c("x","y")], 
       col=c("gold", "deepskyblue", "forestgreen")[viterbi(sealHMMb)], 
       pch=19, cex=0.5)
@
...
<<distExp,echo=FALSE, results="hide", message=FALSE>>=

###### Wrapped Cauchy for turning angle
kappa0wp <- c(0.9, 0.9) # Turning angle concentration parameter, kappa close 
sealHMMwp <- fitHMM(sealPrep, nbState = 2, dist=list(step="gamma", angle="wrpcauchy"), 
...
kappa0 <- c(0.9, 0.9) # Turning angle concentration parameter, kappa close 
sealHMMexp <- fitHMM(sealPrep, nbState = 2, dist=list(step="exp", angle="vm"), 
